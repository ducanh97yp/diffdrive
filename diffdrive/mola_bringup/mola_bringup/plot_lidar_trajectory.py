...
class TrajectoryPlotter(Node):
    def __init__(self):
        ...
        # publishes on it with this pipeline/args (state_estimation_simple module has
        # zero active publishers). '/lidar_odometry/pose' is LidarOdometry's own direct
        # pose output and is what's actually live - confirmed via `ros2 topic info -v`.
        self.create_subscription(Odometry, '/lidar_odometry/pose', self.odometry_callback, 10)
    ...
    def set_mola_parameter(self, param_yaml):
        ...
        # NOTE: can't use rclpy.spin_until_future_complete(self, ...) here - ros_thread
        # (started in main()) already spins this node continuously on another thread,
        # so spinning it again would raise "Executor is already spinning". The
        # background thread's spin will complete the future on its own; just wait for it.
...
class TrajectoryGUI:
    ...
    def update_graphs(self, frame):
        ...
        # NOTE: ros_thread (started in main()) already runs rclpy.spin(node) continuously.
        # Spinning again here from the Tk/matplotlib thread raced with it and raised
        # "RuntimeError: Executor is already spinning" - node.get_data() just reads
        # attributes the background thread's callbacks already keep updated.
